...
# moving in two dimensions. This code implements Protocol A (one-to-one transmission), in which an 
# infected particle can potentially transmit the disease to only one susceptible particle within a contagion radius.  

l=len(sys.argv)

if l<14:
    print("\n Correct syntax is [microscopic_model_protocol_A.py] [path to input config file] [restart? (0/1)] [delta t] [no of timesteps]",\
...
## loop over infected species
   ...
   for p_i in range(num_inf):
       ...
       if(event_flag[0]):
           ...
           if(index_S_c.size>0):
               chk=np.random.choice(len(index_S_c),1);
               rand_S=index_S_c[chk];
               type_list[rand_S] = 1
      
       if(event_flag[1]):
           # statements for I --> R conversion
           if(index_I_c.size>0):
               chk=np.random.choice(len(index_I_c),1);
               rand_I=index_I_c[chk];
               type_list[rand_I] = 2

#### adding in the part that governs the transition from R --> S   
   if(len(pos_rec)>0):
       rec_vec = np.random.binomial(1,(self.rate_relapse*self.dt),num_rec)
       num_relapsed = np.sum(rec_vec)
...
p_file.write("Input syntax: [microscopic_model_protocol_A.py] [path to input config file] [restart? (0/1)] [delta t] [no of timesteps],\
      [D_r] [output folder for results] [save_frequency],\
      [beta] [gamma] [alpha] [s0_frac] [i0_frac] [contag_rad]")
